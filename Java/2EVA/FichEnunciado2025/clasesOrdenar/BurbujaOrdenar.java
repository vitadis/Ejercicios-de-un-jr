...
public class BurbujaOrdenar {
	
	public static <H extends Persona> void ordenarBurbuja(ArrayList<H> personas, PorString criterio) {
		...
		for (int i = 0; i < personas.size() - 1; i++) {
			for (int j = 0; j < personas.size() - 1 - i; j++) {
				...
				// ordenar de la A a Z
				if (criterio.orden(personas.get(j), personas.get(j + 1)) > 0) {
